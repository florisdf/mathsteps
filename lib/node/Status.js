...
// Wraps the node around each step taken. This is useful when steps are done on
// an element of a node (e.g. the steps show how to simplify one of the factors
// in the node). The steps will be put into the node using the given JSON path.
Status.putStepsInNodePath = function(node, path, steps) {
  return steps.map(step => {
    const oldNode = _.cloneDeep(node);
    _.set(oldNode, path, _.cloneDeep(step.oldNode));
    step.oldNode = oldNode;

    const newNode = _.cloneDeep(node);
    _.set(newNode, path, _.cloneDeep(step.newNode));
    step.newNode = newNode;
    if (step.substeps && step.substeps.length > 0) {
      step.substeps = Status.putStepsInNodePath(node, path, step.substeps);
    }
    return step;
  });
};
