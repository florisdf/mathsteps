...
const NodeCreator = {
  operator (op, args, implicit=false, recurse=false) {
    if (args.length === 1) {
      // Only 1 arg, so no operation possible. Return the arg.
      return args[0];
    }
    if (args.length > 2 && recurse) {
      // Recurse to build expression tree
      return NodeCreator.operator(op,
        [args[0], NodeCreator.operator(op, args.slice(1), implicit)],
        implicit);
    }
    switch (op) {
    case '+':
