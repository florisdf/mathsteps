...
Term.getCommonFactorPaths = function(node) {
  const terms = Term.getTerms(node).map(term =>
    NodeType.isUnaryMinus(term)
      ? term.args[0]
      : term);
...
Term.areEqual = function(node1, node2) {
  const facs1 = Factor.fullSplit(math.simplify(simplify(node1)));
  const facs2 = Factor.fullSplit(math.simplify(simplify(node2)));
