...
Term.getFactorCounts = function(node) {
  ...
  factors.forEach(f => {
    ...
    if (knownFacs.length === 0) {
      ...
      let equalFacs = knownFacs.filter(k => Term.areEqual(k, f));
...
Term.areEqual = function(node1, node2) {
  let facs1 = Factor.getFactors(math.simplify(simplify(node1)));
  let facs2 = Factor.getFactors(math.simplify(simplify(node2)));
