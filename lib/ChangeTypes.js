...
module.exports = {
  ...
  // e.g. 3x^2 + 6x -> 3x ( x + 2 )
  ISOLATE_COMMON_FACTOR: 'ISOLATE_COMMON_FACTOR',
  // e.g. 10x^2 + 125y -> (2 * 5) x + (5 * 5* 5) y
  FACTOR_COEFFS_INTO_PRIMES: 'FACTOR_COEFFS_INTO_PRIMES',
  // e.g. 10x^(2m) + 125y -> 10 (x * x * x^m) + 125 y
  FACTORIZE_EXPONENTS: 'FACTORIZE_EXPONENTS',

  // e.g. x^(m + 1) -> x^m * x^1
  SPLIT_EXPONENT_TERMS: 'SPLIT_EXPONENT_TERMS',

  // e.g. x^3 + x^2 + x^1 -> x^(1 + 2) + x^(1 + 1) + x^1
  SPLIT_INTEGER_EXPONENTS: 'SPLIT_INTEGER_EXPONENTS',

  // e.g. a*(x + 4 - 3) -> a*(x + 1)
  SIMPLIFY_FACTORS: 'SIMPLIFY_FACTORS',

  // e.g. x^(3 - m - 2)^2 -> x^((1 - m)*2)
  SIMPLIFY_EXPONENTS: 'SIMPLIFY_EXPONENTS',

  // e.g. x^3^2 -> x^(3*2)
  COLLAPSE_EXPONENTS: 'COLLAPSE_EXPONENTS',
  ...
  // e.g. b*(-(-1 + x)) -> -b * (-1 + x)
  MINUS_OUTSIDE_PARENS: 'MINUS_OUTSIDE_PARENS',

  // e.g. 2*3*3*a + 2*3*b -> 2*3*( 3*a + b )
  EQUAL_FACTORS_OUTSIDE_PARENS: 'EQUAL_FACTORS_OUTSIDE_PARENS',

  // e.g. 3*x^2 + x -> x * (3*x + 1)
  EXPON_FACTORS_OUTSIDE_PARENS: 'EQUAL_FACTORS_OUTSIDE_PARENS',
};
