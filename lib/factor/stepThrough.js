...
function equalizeFacs(node) {
  ...
  opEqFacs.forEach((opEqFac, idx) => {
    opEqFac.oppos.forEach(opFac => {
      setChangeGroup(newNode, opFac.path, idx + 1);
      });
    opEqFac.equal.forEach(eqFac => {
      setChangeGroup(newNode, eqFac.path, idx + 1);
    })
  });
  ...
  opEqFacs.forEach((opEqFac, idx) => {
    opEqFac.oppos.forEach(opFac => {
      ...
      const newFac = Node.Creator.unaryMinus(negFac);
      _.set(newNode, path, newFac);
      setChangeGroup(newNode, path, idx + 1);
    });
  });
  ...
  opEqFacs.forEach((opEqFac, idx) =>
    opEqFac.oppos.forEach(opFac => {
      // The opposite node has a unaryMinus
      // in its top level
      // Remove this minus and change
	  // the sign of the term
	  const termPar = _.get(newNode, Util.getParentPath(opFac.termPath));
	  
      // Go up the tree until we are at a node that is the second arg of a + or - operation
      let termPath = opFac.termPath;
      while (_.last(termPath.split('.')) !== 'args[1]'
             && termPath !== '') {
        termPath = Util.getParentPath(termPath);
      }
      
      let opPath = Util.getParentPath(termPath);
      let opNode = _.get(newNode, opPath);     
...
function isolate(node) {
  ...
  if (facsEqSteps.length > 0) {
    steps.push(new Status(ChangeTypes.EQUALIZE_TERM_FACTORS,
                          oldNode=node,
                          newNode=_.last(facsEqSteps).newNode,
