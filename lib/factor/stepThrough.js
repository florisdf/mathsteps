  ...
  return iterateTermFacsWithStep(node, ChangeTypes.SPLIT_EXPONENT_TERMS,
    (termPath, facPath, oldNode, newNode, term, fac) => {
      ...
      if (expTerms.length > 1) {
        return math.parse(
          expTerms
            .map(exp => Node.Creator.operator('^', [fac.args[0], exp]))
            .map(n => `${n}`)
            .join('*'));
      }
    });
...
function putExponFacsOutsideParensStep(node) {
  ...
  Term.getCommonFactorPaths(tempNode).forEach(term => {
    term.forEach(facPath => {
      ...
      if (NodeType.isConstant(exp, false)
        && Number.isInteger(parseFloat(exp.value))
        && parseInt(exp.value) > 0) {
        // If the exponent is a positive integer, push the facPath to a list of
        // exponents with the same base
        const base = `${fac.args[0]}`;
        if (base in facsWithConstExp) {
          facsWithConstExp[base].push(facPath);
        }
        else {
          facsWithConstExp[base] = [facPath];
        }
      }
    });
  });
  ...
  const minExpPerBase = _.mapValues(facsWithConstExp, facPaths => {
    return facPaths
      .map(path => _.get(newNode, path))
      .map(fac => NodeType.isOperator(fac, '^') ? parseInt(fac.args[1].value) : 1)
      .reduce((acc, curVal) => acc < curVal ? acc : curVal);
  });

  // Split the exponents of the factors with a larger exponent into the
  // smallest exponent + a rest-exponent
  let splitIntExpons = false;
  _.toPairs(facsWithConstExp).forEach(([base, facPaths], idx) => {
    const minExp = minExpPerBase[base];
    facPaths.forEach(facPath => {
      const fac = _.get(newNode, facPath);
      const exp = parseInt(fac.args[1].value);
      if (exp !== minExp) {
        const newExp = Node.Creator.constant(exp - minExp);
        fac.args[1] = Node.Creator.operator('+',
          [Node.Creator.constant(minExp), newExp]);
