...
function equalizeFacs(node) {
  ...
  opEqFacs.forEach((opEqFac, idx) => {
    opEqFac.oppos.forEach(opFac => {
      const fac = opFac.val;
      const negFac = Polynom.negate(_.cloneDeep(fac));
      const negOne = math.parse('(-1)');
      const newFac = Node.Creator.operator('*', [negOne, negFac]);
      newFac.changeGroup = idx;
      modifyObj(fac, newFac);
    })
  });
