...
function equalizeFacs(node) {
  ...
  if (opEqFacs.length === 0) { return steps; }
  
  let opposFacs = opEqFacs.reduce(
    (acc, opeqfac, idx) => {
  ...
  let newTerms = Term.getTerms(node).map(
    (term, tIdx) =>
      Factor.getFactors(term).map(
        (fac, fIdx) => {
          let opposFacs = opposFacs.filter(opfac =>
            opfac.term === tIdx
            && opfac.fac === fIdx);

          if (opposFacs.length > 0) {
            const opFac = opposFacs[0];
            const negFac = Polynom.negate(fac);
            const newFac = Node.Creator.operator('*', ['(-1)', negFac]);
            newFac.changeGroup = opFac.opEqFacIdx;
          } else {
            return fac;
          }
        }
