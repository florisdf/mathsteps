...
Factor.primeFactorize = function(numberNode) {
  ...
  if (NodeType.isConstantOrConstantFraction(numberNode, true)) {
    ...
    if (NodeType.isConstantFraction(numberNode)) {
      ...
      const numerFacs = Factor.primeFactorize(numer);
      const denomFacs = Factor.primeFactorize(denom);
...
Factor.getFactors = function(node){
  ...
  const multFacs = Factor.splitMultcations(node);
