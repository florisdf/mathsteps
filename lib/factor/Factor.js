...
Factor.primeFactorize = function(numberNode) {
  let factors = [];
  Util.modifyNode(numberNode, math.simplify(numberNode));

  if (NodeType.isConstantOrConstantFraction(numberNode, allowUnaryMinus=true)) {
    ...
    if (NodeType.isConstantFraction(numberNode)) {
      ...
      return [
        ...Factor.primeFactorize(numer)
        .filter(f => f.value !== '1'),
        ...Factor.primeFactorize(denom)
        .map(f => Node.Creator.operator('/', args=[math.parse('1'), f]))
      ];
    }
  } else {
