...
Polynom.iterateTermFacs = function(node, callback) {
...
Polynom.iterateTermFacsWithStep = function(node, changeType, callback) {
  const oldNode = node.cloneDeep();
  const newNode = oldNode.cloneDeep();
  let cgrpCount = 0;
  Polynom.iterateTermFacs(newNode, (termPath, facPath, term, fac) => {
